...
class MrMs:
    ...
    def setRate(self, rate=0):
        ...
        try:
            ...
            # set rate
            if(rate == 0):
                self.rate = random.randint(1, 1000)
            else:
                self.rate = rate
...
class Rate:
    ...
    def __init__(self, *persons):  # initialize class
        try:
            if(len(persons) == 0):
                raise Ex(4)  # exceptions handling
    ...
    def showWinner(self):
        ...
        for i in range(0, len(self.candidates)):
            if(self.candidates[i].rate > winner.rate):
                # set person with highest rating as winner
    ...
    def candidatesSort(self, by="rate"):
        ...
        print("-------------- Rate Table --------------")
        for i in range(0, len(self.candidates)):
            print(rateTable[i].name + " " + str(rateTable[i].age)
                  + " years, rate: " + str(rateTable[i].rate))
